...
class ImportStocks extends Command
{
    ...
    protected $description = 'Импорт заказов (stocks) для всех аккаунтов и сервисов';

    public function handle()
    {
        $this->info('Старт импорта stocks...');
        ...
        if ($tokens->isEmpty()) {
            $this->warn('Нет токенов для импорта stocks.');
            return 0;
        }
        ...
        foreach ($tokens as $token) {
            ...
            $lastDate = Stock::where('account_id', $accountId)->max('date') ?? '2024-01-01';
            ...
            while ($page <= $maxPages) {
                $response = Http::get("$serviceUrl/stocks", [
                    'dateFrom' => $lastDate,
        ...
        $this->info('Импорт stocks завершён.');
        return 0;
    }
}
